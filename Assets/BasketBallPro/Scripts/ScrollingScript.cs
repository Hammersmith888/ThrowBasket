namespace GameBench
{
    ...
    public class ScrollingScript : MonoBehaviour
    {
        ...
        internal bool isLinkedToCamera = false;
        internal bool isLooping = true;
        private List<SpriteRenderer> backgroundPart;
        private Vector2 repeatableSize;
        internal HoopData[] hoopsList;
        void Awake()
        {
            hoopsList = GetComponentsInChildren<HoopData>();
            ResetData();
            SetActive(false);
        }
        void ResetData()
        ...
        internal void ResetDefault(bool move = false, bool activate = false)
        {
            Move = move;
            if (!activate)
            {
                ...
                transform.GetChild(1).localPosition = new Vector2(12.79f, 0);
                transform.GetChild(2).localPosition = new Vector2(25f, 0);
            }
            ResetData();
            SetActive(activate);
        }
        ...
        internal void SetActive(bool first)
        {
            gameObject.SetActive(first);
        }
    }
}